...
Everything is explained in the [Merge, join and concatenate](https://pandas.pydata.org/docs/user_guide/merging.html) article of the documentation, still it's a _very_ (very!) long article that can't really be read at once hoping to understand/remember everything.
...
:information_source: Pandas has a whole article in the documentation about [Working with missing data](https://pandas.pydata.org/docs/user_guide/missing_data.html). Again it's a pretty long article that you don't need to read now, but keep it in mind next time you are exploring a Dataset with a lot of `NaN`.
...
Let's now use [`pandas.DataFrame.join()`](https://pandas.pydata.org/docs/reference/api/pandas.DataFrame.join.html):
...
There's a third way to put two dataframes together, using [`pandas.concat()`](https://pandas.pydata.org/docs/reference/api/pandas.concat.html). Let's jump right into it:
...
You will have to use [`pandas.DataFrame.rename()`](https://pandas.pydata.org/docs/reference/api/pandas.DataFrame.rename.html) function on your winter and summer dataframes.
...
Time to perform an inner merge of `countries_df` and `summer_df` (into a new dataframe `summer_countries_df`). As we'll want to merge all games into one DataFrame at the end, **add a `Season`** column to the `summer_countries_df`.
...
:bulb: **Hint 1** The [`pandas.DataFrame.groupby()`](https://pandas.pydata.org/docs/reference/api/pandas.DataFrame.groupby.html) can group over a **`list`** of columns.

:bulb: **Hint 2** You need to use the [`pandas.DataFrame.unstack()`](https://pandas.pydata.org/docs/reference/api/pandas.DataFrame.unstack.html) function
...
The same goes for mixed events, they are counted double. How could you remove those too?
